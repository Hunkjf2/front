...
export class ListarPerfilComponent extends FiltroComponent implements OnInit {
    ...
    private carregarPerfis(): void {
        this.perfilService.listarTodos().subscribe({
            next: (perfis: Perfil[]) => this.dadosTabela.data = perfis,
            error: (error: any) => {
    ...
    public deletarPerfil(id: string): void {
